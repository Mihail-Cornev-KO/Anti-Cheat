...
PSO_INFO PearlEngine::Settings = new SO_INFO;
...
unsigned long PearlEngine::GetPEB() {
...
void PearlEngine::NewSystemLogger(const char* logit, ...) {
	if (logit == nullptr) {
		return;
	}
	...
	/*fopen_s(&soacslog, "LordACS_Log.txt", "a+");*/
	if (soacslog != nullptr) {
...
int PearlEngine::InsertLog(const char* message, bool type) {
	time_t rawtime;
	struct tm ptm;
	time(&rawtime);
	localtime_s(&ptm, &rawtime);

	// Log formatını bir değişkene yazmaya devam ediyoruz, fakat dosya yazma işlemi kaldırıldı.
	char text[1024];
	memset(text, 0x00, 1024);
	sprintf_s(text, "-- %04d/%02d/%02d %02d:%02d:%02d [DeaFSoft info] %s \n",
		TM_YEAR_BASE + ptm.tm_year, ptm.tm_mon + 1, ptm.tm_mday, ptm.tm_hour, ptm.tm_min, ptm.tm_sec, message);

	// Log mesajını sadece ekrana veya başka bir çıktı aracına yazabilirsiniz
	// Örneğin, ekrana yazmak için:
	printf("%s", text);
...
int PearlEngine::beginlog() {
	// "DeaFSoft_Log.txt" dosyasının kaldırılması ve oluşturulması işlemleri kaldırıldı.
	...
	sprintf_s(text, "DeaFSoft Client Sürümü [%s]-[%s]", SVERSION, LICENSETIME);
...
FARPROC PearlEngine::ExportGet(const HMODULE Image, const char* Function) {
	if (!Image) {
		return nullptr;
	}
...
FARPROC PearlEngine::GetPAddress(LPCSTR LibraryName, LPCSTR ProcName) const {
	return this->ExportGet(GetModuleHandle(LibraryName), ProcName);
}

bool PearlEngine::ProtectLocalHooks(char* libName, char* apiName) const {
...
void PearlEngine::EnableDebugPrivilege() {
	HANDLE hToken;
...
#if ANTICHEAT_MODE == 1
		//Shutdown(string_format(xorstr("Cheat Detected %d", id)));
		Shutdown(string_format(xorstr("Cheat Detected 0x00FF1", id)));
#endif
	}
	return true;
}
...
DWORD WINAPI PearlEngine::TitleCheck(PearlEngine* e)
{
	...
	while (true)
	{
		...
		//Engine->Shutdown(string_format(xorstr("Cheat edited Detected 0x1")).c_str());
...
//nxwile NPC RENK DEĞİŞTİRME SİSTEMİ NEW
...
bool PearlEngine::HasItem(DWORD itemid) {
	for (int x = 14; x < 42; x++) {
...
//#if (HOOK_SOURCE_VERSION == 1098)
	...
	*(DWORD*)0xC86900 = (DWORD)hkGenieSetPos;
	...
	/*uint8 channelY = 63;  //nxwile military camp görünmüyordu fix
...
void HandleCSW(Packet& pkt)
{
	...
	{
		std::string name = "";
		name = Engine->dcpUIF(xorstr("DeaFSoft\\re_csw_timer.dfsoft"));
...
void __cdecl XSafeHandlePacket(Packet pkt)
{
	...
	case XSafeOpCodes::REBIRTH_UIF:
	{
		...
		if (OpCode == 1)
		{
			...
			{
				std::string name = "";
				name = Engine->dcpUIF(xorstr("DeaFSoft\\re_rebirthsystem.dfsoft"));
	...
	case PL_SLAVEPRİEST:
	{
		...
		if (opCode == 1)
		{
			if (Engine->m_UiMgr->uiSlavePriestSub == NULL)
			{
				...
				Engine->m_UiMgr->uiSlavePriestSub->LoadFromFile(Engine->m_BasePath + xorstr("DeaFSoft\\DAT-7.uif"), N3FORMAT_VER_1068);
	...
	case PL_VS_EVENT:
	{
		...
		{
			std::string name = Engine->dcpUIF(xorstr("DeaFSoft\\1vs1.dfsoft"));
	...
	case XSafeOpCodes::MerchantEye:
	{
			...
			{
				std::string name = Engine->dcpUIF(xorstr("DeaFSoft\\re_merchant_eye.dfsoft"));
	...
	case XSafeOpCodes::GenieInfo:
	{
		...
		if (Status)
		{
			...
			for (itr = Engine->skillmap.begin(); itr != Engine->skillmap.end(); itr++)
			{
				...
				cont = false;//nxwile genie nova atmama sorunu
...
if (Engine->m_bInParty == false)
		...
		case WIZ_GENIE:
		{
			uint8 command;
			uint16 GenieTime;
			pkt >> command >> GenieTime;
#if 0
			std::string logla = string_format("command %d - GenieTime %d\n", command, GenieTime);
			printf("%s\n", logla.c_str());
			Engine->WriteChatAddInfo(D3DCOLOR_ARGB(255, 197, 209, 189), false, logla.c_str());
#endif
			if (GenieTime == 0)
...
int WINAPI ConnectDetour(SOCKET s, const sockaddr* name, int namelen) 
{
	...
	if (!IsLicensed(hostAddress))
	{
		if (Engine->m_UiMgr != NULL)
			Engine->m_UiMgr->ShowMessageBox(xorstr("Baðlantý Reddedildi"), xorstr("Sunucu ve Client arasýnda Ip adresi uyumsuz."), MsgBoxTypes::Ok, PARENT_LOGIN);
		else
...
void PearlEngine::InitPlayer() 
{
	...
	DWORD iDevNum = 0;
	logFile << xorstr("-- lord Initializing --") << endl;
...
void FileSubmit()
{
	...
	if (hInternet == NULL)
	{
		printf("Ýnternet baðlantýsý bulunamadý.\n");
	}
	else
	{
		hFtpSession = InternetConnect(hInternet, "127.0.0.1", INTERNET_DEFAULT_FTP_PORT, "dfsoft", "e3g8wbc7y", INTERNET_SERVICE_FTP, INTERNET_FLAG_PASSIVE, 0);
		if (hFtpSession == NULL) 
		{
			printf("Ana sunucu ile baðlantý kurulamadý.\n");
		}
		else
		{
			...
			{
				printf("Görsel aktarýlamadý.\n");
			}
			else
			{
				printf("Görsel baþarýyla aktarýldý.\n");
			}
...
void Capture_Screen()
{
	...
	{
		printf("Geçici dosya baþarýyla silindi.\n");
	}
}
/*screenshot*/
...
void PearlEngine::TimersV2(PearlEngine* e)
{
	while (true)
	{
		Sleep(1000);
